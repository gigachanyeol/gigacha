...
public class Room_JUnitTest {
	...
	public void insertRoom_test() {
		RoomDto dto = RoomDto.builder().room_id("ROOM022").room_name("회의실 D").capacity(7).open_hours("09:00")
				.close_hours("18:00").created_at("2025-03-03").updated_at("2025-03-03").use_yn("Y")
				.image_url("/images/roomD.png").build();
	}
	...
	// 회의실 정보 수정
	@Test
	public void testUpdateRoom_WithRoomNameAndCapacity() {
		RoomDto dto = new RoomDto();
		dto.setRoom_id("ROOM024"); // 기존에 존재하는 회의실 ID
//		dto.setRoom_name("회의실 C");
//		dto.setCapacity(10);
		
		dto.setUse_yn("N");

		int updatedRows = roomDao.updateRoom(dto);
