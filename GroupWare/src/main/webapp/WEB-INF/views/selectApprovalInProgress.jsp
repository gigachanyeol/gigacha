			...
			<table class="table table-hover">
				...
				<c:forEach items="${approvalList}" var="dto">
					<tr>
						...
						<td><c:if test='${dto.approval_status eq "ST01"}'>임시저장</c:if>
							<c:if test='${dto.approval_status eq "ST02"}'>결재대기</c:if> <c:if
								test='${dto.approval_status eq "ST03"}'>결재진행중</c:if> <c:if
								test='${dto.approval_status eq "ST04"}'>결재완료</c:if> <c:if
								test='${dto.approval_status eq "ST05"}'>반려</c:if></td>
